...
router
    ...
    .put('/buy', bodyParser, (req, res, next) => {
        const {stock, price} = req.body; //have object be {stock:ticker, shares:number, price:cost}?
        const userId = req.user.id;
        Promise.all([
            Portfolio
                .findOne({userId})
                .then(portfolio => {
                    ...
                    if(stock[0] in portfolio.stocks) portfolio.stocks[stock[0]] += stock[1];
                    else portfolio.stocks[stock[0]] = stock[1];
